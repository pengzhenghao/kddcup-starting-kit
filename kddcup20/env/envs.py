"""Modified from https://github.com/illidanlab/Simulator"""
import logging

from kddcup20.env.objects import *
from kddcup20.env.utilities import *

# from algorithm import *

# current_time = time.strftime("%Y%m%d_%H-%M")
# log_dir = "/nfs/private/linkaixiang_i/data/dispatch_simulator/experiments
# /"+current_time + "/"
# mkdir_p(log_dir)
...
class CityReal:
    ...
    def __init__(
        ...
        :param order_num_dist: 144 [{node_id1: [mu, std]}, {node_id2: [mu,
        std]}, ..., {node_idn: [mu, std]}]
                            node_id1 is node the index in self.nodes
        :param idle_driver_dist_time: [[mu1, std1], [mu2, std2], ..., [mu144,
        std144]] mean and variance of idle drivers in
        the city at each time
        :param idle_driver_location_mat: 144 x num_valid_grids matrix.
        :param order_time_dist: [ 0.27380797,..., 0.00205766] The probs of
        order duration = 1 to 9
        :param order_price_dist: [[10.17, 3.34],   # mean and std of order's
        price, order durations = 10 minutes.
                                   [15.02, 6.90],  # mean and std of order's
                                   price, order durations = 20 minutes.
                                   ...,]
        :param onoff_driver_location_mat: 144 x 504 x 2: 144 total time
        steps, num_valid_grids = 504.
        mean and std of online driver number - offline driver number
        onoff_driver_location_mat[t] = [[-0.625       2.92350389]  <--
        Corresponds to the grid in target_node_ids
        ...
        self.l_max = l_max  # Start from 1. The max number of layers an order
        # can across.
        assert l_max <= M - 1 and l_max <= N - 1
        assert 1 <= l_max <= 9  # Ignore orders less than 10 minutes and
        # larger than 1.5 hours
        ...
        # [[92, 300, 143, 2, 13.2],...] origin grid, destination grid,
    ...
    def construct_node_real(self, mapped_matrix_int):
        """ Initialize node, only valid node in mapped_matrix_in will be
        initialized.
        """
    ...
    def reset(self):
        """ Return initial observation: get order distribution and idle
        driver distribution
    ...
    def reset_clean(self, generate_order=1, ratio=1, city_time=""):
        """ 1. bootstrap oneday's order data.
            2. clean current drivers and orders, regenerate new orders and
            drivers.
            can reset anytime
        :return:
        """
    ...
    def utility_add_driver_real_new_offlinefirst(self, num_added_driver):
        ...
        # curr_idle_driver_distribution = self.get_observation()[0][np.where(
        # self.mapped_matrix_int > 0)]
        curr_idle_driver_distribution = self.get_observation()[0]
        curr_idle_driver_distribution_resort = np.array(
            [
    ...
    def utility_set_drivers_offline_real_new(self, n_drivers_to_off):
        ...
        idle_diff = curr_idle_driver_distribution_resort - \
                    idle_driver_distribution.astype(
                        int)
        idle_diff[np.where(idle_diff <= 0)] = 0
    ...
    def step_driver_status_control(self):
        # Deal with orders finished at time T=1, check driver status. finish
        # order, set back to off service
        for key, _driver in self.drivers.iteritems():
    ...
    def step_driver_online_offline_control_new(self, n_idle_drivers):
        ...
        :param n_idle_drivers: the number of idle drivers expected at current
        moment
        :return:
        """
    ...
    def step_assign_order(self):
        ...
        for node in self.nodes:
            if node is not None:
                ...
                reward_node, all_order_num_node, finished_order_num_node = \
                    node.simple_order_assign_real(
                        self.city_time, self)
                reward += reward_node
    ...
    def step_assign_order_broadcast_neighbor_reward_update(self):
        """ Consider the orders whose destination or origin is not in the
        target region
        :param num_layers:
            ...
            if node is not None:
                if node.order_num != 0:
                    reward_node_broadcast, finished_order_num_node_broadcast \
                        = node.simple_order_assign_broadcast_update(self,
                                                                    neighbor_reward)
    ...
    def step(self, dispatch_actions, generate_order=1):
        ...
        self.step_driver_status_control()  # drivers finish order become
        # available again.
