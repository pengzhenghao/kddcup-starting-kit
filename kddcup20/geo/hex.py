"""
https://github.com/stephanh42/hexutil/blob/master/hexutil/__init__.py
"""
import math
import operator
import random
...
class Hex(namedtuple("Hex", "x y")):
    ...
    def rotate_left(self):
        """Given a hex return the hex when rotated 60° counter-clock-wise
        around the origin.
        """
        x, y = self
    ...
    def rotate_right(self):
        """Given a hex return the hex when rotated 60° clock-wise around the
        origin.
        """
        x, y = self
    ...
    def field_of_view(self, transparent, max_distance, visible=None):
        ...
        visible.
        The bitmask is useful if you want to use this function also to
    ...
    def find_path(self, destination, passable, cost=lambda pos: 1):
        ...
        passable    -- Function of one position, returning True if we can
        move through this hex.
        cost        -- cost function for moving through a hex. Should return
        a value ≥ 1. By default all costs are 1.
        """
...
class HexGrid(namedtuple("HexGrid", "width height")):
    ...
    The hexgrid is determined by width and height, which are the screen
    coordinates
...
class HexPathFinder:
    ...
    done  -- True if path-finding is complete: we either found a path or know
    there isn't one
    path  -- The path, as a tuple of positions from start to destination (
    including both). Empty tuple if found is False.
    """
    ...
    def run(self):
        """Run path-finding until done, that is, we either found a path or
